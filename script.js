	...
	class FileManager {
		...
		updateFileTree() {
            ...
            <li class="${
												...
												}" data-file="${sanitizeInput(
						name
					)}" role="button" aria-label="Select file ${sanitizeInput(name)}">
              <i class="fas fa-file-code"></i> ${sanitizeInput(name)}
              <button class="delete-file" data-file="${sanitizeInput(
															name
														)}" style="margin-left: 10px; background: #dc3545; padding: 2px 6px;" aria-label="Delete file ${sanitizeInput(
						name
					)}">
...
<body>
	...
	function addNewFile() {
		try {
			const name = prompt(
				"Enter file name (must end with .js, .html, .css, .json, or .png):"
			);
